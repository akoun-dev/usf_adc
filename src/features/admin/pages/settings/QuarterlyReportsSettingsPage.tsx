...
import {
    useQuarterlyReports,
    useCreateQuarterlyReport,
    useUpdateQuarterlyReport,
    useDeleteQuarterlyReport,
    QuarterlyReport,
...
export default function QuarterlyReportsSettingsPage() {
    ...
    const { data: reports = [], isLoading } = useQuarterlyReports()
    ...
    const [form, setForm] = useState<
        Omit<QuarterlyReport, "id" | "created_at">
    >({
        title: "",
        quarter: "Q1",
    ...
    const downloadReport = (report: QuarterlyReport) => {
        if (!report.file_url) {
            toast({
                title: t("admin.noFile", "Aucun fichier associé"),
                variant: "destructive",
            })
            return
        }
        ...
        <div className="space-y-6 animate-fade-in">
                ...
                <div className="space-y-1">
                    ...
                    <p className="text-muted-foreground">
                        {t(
                            ...
                            "Gérez les rapports d'activité trimestriels de l'organisation"
                        )}
                    </p>
                </div>
            </div>
                ...
                <Card>
                    ...
                    <CardContent>
                        ...
                        <p className="text-xs text-muted-foreground">
                            {reports.filter(r => r.is_published).length}{" "}
                            {t("admin.published", "publiés")}
                        </p>
                    </CardContent>
                </Card>
            ...
            <Card>
                <CardHeader>
                    <CardTitle>
                        {editingId
                            ? t("admin.editReport", "Modifier le rapport")
                            : t("admin.createReport", "Créer un rapport")}
                    </CardTitle>
                    <CardDescription>
                        {editingId
                            ? t(
                                  "admin.editReportDesc",
                                  "Modifiez les détails du rapport trimestriel"
                              )
                            : t(
                                  "admin.createReportDesc",
                                  "Créez un nouveau rapport trimestriel"
                              )}
                    </CardDescription>
                </CardHeader>
                <CardContent className="space-y-4">
                    ...
                    <div className="space-y-2">
                        ...
                        <Textarea
                            placeholder={t(
                                "admin.reportSummaryPlaceholder",
                                "Résumé des activités du trimestre..."
                            )}
                            value={form.summary}
                            onChange={e =>
            ...
            <Card>
                <CardHeader>
                    <CardTitle>
                        {t("admin.reportsList", "Liste des rapports")}
                    </CardTitle>
                    <CardDescription>
                        {t(
                            "admin.reportsListDesc",
                            "Historique de tous les rapports trimestriels"
                        )}
                    </CardDescription>
                </CardHeader>
                <CardContent>
