...
export function PlatformSettingsTab() {
  ...
  const getValue = (setting: PlatformSetting) =>
    localValues[setting.id] !== undefined ? localValues[setting.id] : setting.value;
  ...
  const handleSave = (setting: PlatformSetting) => {
    ...
    updateSetting.mutate(
      { id: setting.id, value: value as import('@/integrations/supabase/types').Json },
      {
        onSuccess: () => {
          setLocalValues((prev) => {
            const next = { ...prev };
          ...
          toast({ title: t('admin.settingUpdated', { label: setting.label }) });
        },
        onError: () => toast({ title: t('admin.saveError'), variant: 'destructive' }),
      }
    );
  };
          ...
          <Switch
            ...
            onCheckedChange={(checked) => {
              updateSetting.mutate(
                ...
                {
                  onSuccess: () => toast({ title: t('admin.settingUpdated', { label: setting.label }) }),
    ...
    return (
      <div className="space-y-2" key={setting.id}>
        ...
        <div className="flex gap-2">
          {isLongText ? (
            ...
            <Input
              ...
              onChange={(e) =>
                handleChange(
                  setting.id,
                  typeof setting.value === 'number' ? Number(e.target.value) : e.target.value
                )
              }
              className="flex-1"
            />
          ...
          {isDirty && (
            <Button size="icon" onClick={() => handleSave(setting)} disabled={updateSetting.isPending}>
  ...
  return (
    <Card>
      ...
      <CardContent className="space-y-6">{generalSettings.map(renderField)}</CardContent>
    </Card>
  );
}