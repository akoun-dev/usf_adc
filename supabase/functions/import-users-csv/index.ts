...
const VALID_ROLES = ["public_external", "point_focal", "country_admin", "global_admin"];
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (!roleCheck) {
      return new Response(JSON.stringify({ error: "Forbidden: global_admin required" }), {
        status: 403,
        headers: { ...corsHeaders, "Content-Type": "application/json" },
      });
    }
    ...
    for (let i = 0; i < rows.length; i++) {
      ...
      if (!row.email || !/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(row.email.trim())) {
        results.push({ row: rowNum, email: row.email || "", status: "error", error: "Email invalide" });
        continue;
      }
      ...
      if (!VALID_ROLES.includes(role)) {
        results.push({ row: rowNum, email: row.email, status: "error", error: `Rôle invalide: ${row.role}` });
        continue;
      }
      ...
      if (countryCode && !countryId) {
        results.push({ row: rowNum, email: row.email, status: "error", error: `Pays inconnu: ${countryCode}` });
        continue;
      }
      ...
      try {
        ...
        if (createError) {
          results.push({ row: rowNum, email: row.email, status: "error", error: createError.message });
          continue;
        }
