...
export function RoleManager({
    ...
    const isOnlyPublicExternal =
        currentRoles.length === 1 && currentRoles[0] === "point_focal"
    const canPromote =
        isOnlyPublicExternal && allowedRoles.includes("country_admin")
            ...
            {canPromote && (
                <AlertDialog>
                    ...
                    <AlertDialogContent>
                        ...
                        <AlertDialogFooter>
                            <AlertDialogCancel>
                                {t("users.promoteCancel")}
                            </AlertDialogCancel>
                            <AlertDialogAction
                                onClick={() => {
                                    addRole.mutate(
                                        { userId, role: "point_focal" },
                                        {
                                            onSuccess: () => {
                                                supabase.functions
                                                    .invoke(
                                                        "notify-role-promotion",
                                                        {
                                                            body: {
                                                                user_id: userId,
                                                            },
