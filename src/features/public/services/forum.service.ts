...
export async function getPublicForumTopics(): Promise<ForumTopic[]> {
  ...
  try {
    // Try to get authors - this works for authenticated users
  ...
  // Get all posts counts in one query
  ...
  const topicsWithCounts = topics.map(topic => ({
    ...topic,
    category: topic.category_id ? categoryMap.get(topic.category_id) || null : null,
...
export async function getPublicForumTopicById(topicId: string): Promise<ForumTopic | null> {
  const { data, error } = await supabase
    .from('forum_topics')
    .select('*')
    .eq('id', topicId)
