...
export function InviteUserForm() {
  ...
  const roleLabels: Record<AppRole, string> = {
    public_external: t('invitations.roles.public_external', 'Public externe'),
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div className="space-y-2">
            ...
            <Select value={countryId} onValueChange={setCountryId}>
              <SelectTrigger><SelectValue placeholder={t('invitations.selectCountry')} /></SelectTrigger>
              <SelectContent>
                {(countries || []).map((c) => (
                  <SelectItem key={c.id} value={c.id}>{c.code_iso} — {c.name_fr}</SelectItem>
                ))}
              </SelectContent>
            </Select>
          </div>
          <Button type="submit" className="w-full" disabled={isPending}>
            {isPending ? t('common.loading') : t('invitations.sendInvitation')}
          </Button>
        </form>
      </DialogContent>
    </Dialog>
