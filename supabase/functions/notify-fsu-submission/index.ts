...
Deno.serve(async req => {
    ...
    try {
        ...
        // 4. Get admin recipients (country_admin of same country + all super_admin)
        const { data: adminRoles } = await adminClient
            .from("user_roles")
            .select("user_id, role")
            .in("role", ["country_admin", "super_admin"])
        ...
        // Filter: super_admin always, country_admin only if same country
        const recipients: {
            userId: string
            phone: string | null
            email?: string
            ...
            <div style="font-family:Arial,sans-serif;max-width:600px;margin:0 auto;padding:20px;background:#f8fafc;">
              ...
              <div style="border-top:1px solid #e2e8f0;padding:16px 0;text-align:center;color:#94a3b8;font-size:12px;">
                © ${new Date().getFullYear()} ATU/UAT — Plateforme FSU Afrique
                    ...
                    console.error(
                        `Email failed for ${recipient.userId}:`,
                        emailErr
                    )
                }
            }
